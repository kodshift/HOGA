...
function Userprofilesdetails() {
  ...
  const handleSubmit = async () => {
    try {
      const response = await fetch(`https://expertapp-86aba-default-rtdb.firebaseio.com/hogamilan/${userData.uniqueid}.json`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ ...userData, ...editedData }), // Merge userData and editedData
      });
      ...
      // Update local storage with edited data
      localStorage.setItem('userData', JSON.stringify({ ...userData, ...editedData }));
      setUserData({ ...userData, ...editedData }); // Update state with merged data
      setEditedData({}); // Clear edited data state
      alert("your profile should be updated")
    } catch (error) {
        ...
        <Drawer      className="custom-drawer"  anchor="right" open={drawerOpen} onClose={toggleDrawer(false)}>
          ...
          >
            <ChevronLeftIcon   style={{color: "orange", width:"40px", height:"40px" }} />
              ...
              >
                <button style={{width:"200px", borderRadius:"20px", height:"40px", backgroundColor:"red",borderColor:'white', color:'white'}}   >Home</button>
              </ListItem>
